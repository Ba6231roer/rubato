...
class SkillManager(SkillLoader):
    ...
    async def _load_from_dir(
        ...
        """从指定目录加载 Skills，复用父类 _load_skills_from_dir 并跳过已注册的"""
        return self._load_skills_from_dir(dir_path, skip_existing=True)
        ...
        for conditional_skill in self.conditional_skills:
            ...
            for file_path in file_paths:
                if conditional_skill.matches(file_path, self.cwd):
                    ...
                    break
            if not matched:
                remaining.append(conditional_skill)

        self.conditional_skills = remaining
        return activated
    ...
    def get_all_active_skills(self) -> List[SkillMetadata]:
        ...
        base_names = {s.name for s in base_skills}

        all_skills = list(base_skills)
        for skill in self.dynamic_skills:
            if skill.name not in base_names:
