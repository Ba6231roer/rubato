class KnowledgeManager {
    constructor() {
        ...
        this.elements = {
            ...
            editor: document.getElementById('knowledgeEditor'),
            mindmap: document.getElementById('knowledgeMindmap'),
            saveBtn: document.getElementById('saveKnowledgeBtn'),
            sidebar: document.getElementById('knowledgeSidebar'),
            sidebarToggle: document.getElementById('knowledgeSidebarToggle'),
            sidebarExpand: document.getElementById('knowledgeSidebarExpand'),
            editorPanel: document.getElementById('knowledgeEditorPanel'),
            mindmapPanel: document.getElementById('knowledgeMindmapPanel'),
            resizeHandle: document.getElementById('knowledgeResizeHandle')
        };
    ...
    async loadTree() {
        try {
            this.tree = await API.getKnowledgeTree();
            this.renderTree();
        } catch (error) {
    ...
    renderTree() {
        if (!this.tree || this.tree.length === 0) {
            this.elements.tree.innerHTML = '<div class="tree-empty">暂无知识库文件</div>';
            return;
        }
    ...
    async selectFile(fileItem) {
        document.querySelectorAll('#knowledgeTree .tree-item.file').forEach(item => {
        ...
        try {
            const data = await API.getKnowledgeFile(path);
    ...
    async saveFile() {
        ...
        try {
            const result = await API.updateKnowledgeFile(this.currentFile, content);
            if (result.success) {
