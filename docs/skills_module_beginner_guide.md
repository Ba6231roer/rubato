...
### 柜子二：内容缓存 `skill_contents: OrderedDict[str, str]`

这个有序字典缓存了 Skill 的完整正文内容，但它有一个容量限制——默认最多只放 **3 个** Skill 的内容。这就是所谓的 **LRU 缓存**。

> 💡 **什么叫 "LRU 缓存"？**
> LRU = Least Recently Used，"最近最少使用"。想象你有一张书桌，上面最多只能放 3 本书。你每次要看一本书，就把它放到桌面上。如果桌面已经满了，你就把那本**最久没翻过的**书放回书架，腾出位置给新书。
>
> 这里的 LRU 缓存就干了同样的事：
> - 每次你读取某个 Skill 的内容，它就被标记为"最近使用过"（`move_to_end`，挪到队伍末尾）
> - 当缓存满了要存新内容时，排在队伍**最前面**的那个（即最久没被用过的）就被踢出去（`popitem(last=False)`）
>
> **为什么需要这个？** 因为 Skill 的正文可能很长（有些几百行甚至更多），如果所有 Skill 的全文都放在内存里，既浪费空间又没必要——大部分 Skill 在当前对话中根本用不到。所以只保留最近用过的几个，需要时再从文件重新读。
...
> 💡 **白名单机制**：`enabled_skills` 是一个可选的"白名单"。如果设置了白名单（比如 `{"test-execution"}`），那么只有名字在这个白名单里的 Skill 才会被加载，其他的一律跳过。如果白名单为空（默认），则加载全部 Skill。

### 第二步：对话中——按需加载正文

当对话中需要某个 Skill 的完整内容时，`load_full_skill(skill_name)` 被调用：

1. 先查 Registry 的内容缓存——如果已经有了，直接返回（同时更新 LRU 顺序）
...
6. 从磁盘读取文件 → 剥离 YAML 头 → 存入 LRU 缓存 → 返回正文
...
SkillRegistry  ←──注册元数据──  │
     │                          │
     ▼                          ▼
SkillLoader  ──从磁盘读取──→  Registry（存元数据 + LRU缓存内容）
     │
     ▼
