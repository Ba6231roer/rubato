...
关键方法：`mark_skill_loaded()` 标记 Skill 已加载并同步 registry 引用时间戳；`is_skill_loaded()` 检查 Skill 是否曾经加载过（基于 `_loaded_skills` 历史列表，**不感知 Skill 驱逐**）。辅助方法：`clear()`, `get_loaded_skills()`, `get_context()`, `add_context()`, `update_context()`, `set_registry()`。

> **注意**：判断 Skill 是否当前存在于系统提示词中应使用 `SystemPromptRegistry.has_skill()`，而非 `is_skill_loaded()`。详见 [4.7 Skill 驱逐与重新加载流程](#47-skill-驱逐与重新加载流程)。
...
```mermaid
flowchart TD
    A["_run_compression_pipeline()"] --> A1["第0级：Skill 驱逐<br/>remove_stale_skills(max_age_seconds)"]
    A1 --> B["第1级：boundary截取<br/>get_messages_after_compact_boundary"]
...
Skill 在上下文压缩管线第 0 级被驱逐后，支持两条路径自动重新加载：

```mermaid
flowchart TD
    A["压缩管线第 0 级<br/>remove_stale_skills(max_age_seconds)"] --> B["超时未引用的 Skill<br/>从 _sections 中移除"]
    B --> C["Skill 被驱逐出系统提示词<br/>has_skill() 返回 False"]

    C --> D["重新加载路径 1：<br/>Agent 入口点（用户消息触发）"]
    C --> E["重新加载路径 2：<br/>ReAct Loop（LLM 输出触发）"]

    D --> D1["find_matching_skill(user_input)"]
    D1 --> D2{has_skill(skill_name)?}
    D2 -->|"False：已被驱逐"| D3["load_full_skill → add_skill<br/>重建系统提示词 + QueryEngine"]
    D2 -->|"True：仍在提示词中"| D4["mark_skill_referenced<br/>刷新引用时间戳"]

    E --> E1["_check_and_load_skills_from_llm_output"]
    E1 --> E2["skill_find_func(llm_output)"]
    E2 --> E3["_on_skill_needed(skill_name)"]
    E3 --> E4{has_skill(skill_name)?}
    E4 -->|"False：已被驱逐"| E5["load_full_skill → add_skill<br/>重建系统提示词 + QueryEngine"]
    E4 -->|"True：仍在提示词中"| E6["mark_skill_referenced<br/>刷新引用时间戳"]
```

**关键设计决策**：Agent 入口点（`submit` / `stream_submit` / `stream_submit_sdk`）和 ReAct Loop 回调（`_on_skill_needed_from_loop`）中的 Skill 加载判断均使用 `_system_prompt_registry.has_skill()` 而非 `context_manager.is_skill_loaded()`：

| 方法 | 检查语义 | 驱逐后返回值 |
|------|----------|-------------|
| `is_skill_loaded()` | Skill 是否曾经加载过（历史记录） | True（无法检测驱逐） |
| `has_skill()` | Skill 是否当前存在于系统提示词中 | False（正确检测驱逐） |

使用 `is_skill_loaded()` 会导致被驱逐的 Skill 永远无法重新加载，因为该方法仅检查 `_loaded_skills` 历史列表，无法感知 `remove_stale_skills` 的移除操作。
